...
class Docs extends CI_Controller
{
    ...
    public function print()
    {
        if (!empty($this->input->get('id_absen'))) {
        ...
        }else if (!empty($this->input->get('id_izin'))) {
            $id_izin = $this->input->get('id_izin');
            $querydata = $this->db->query("select * from tb_izin left join user on tb_izin.nama = user.id_pegawai where id = '$id_izin'")->row_array();
            $data = [
                'dataapp' => $this->get_datasetupapp,
                'dataizin' => $querydata
            ];
            ob_clean();
            $mpdf = new \Mpdf\Mpdf();
            $html = $this->load->view('layout/print/printizin', $data, true);
            //$pdfFilePath = "storage/pdf_cache/absensipegawai_" . time() . "_download.pdf";
            $stylesheet = file_get_contents(FCPATH . 'assets/css/mpdf-bootstrap.css');
            $mpdf->WriteHTML($stylesheet, \Mpdf\HTMLParserMode::HEADER_CSS);
            $mpdf->WriteHTML(utf8_encode($html), \Mpdf\HTMLParserMode::HTML_BODY);
