...
class Ajax extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        // $this->Waktu = $this->db->get_where('db_hari', ['nama_hari' => $hari[(int)date("w")]])->row();
    ...
    {
        echo json_encode($this->Waktu->jam_masuk);
    }
    ...
    public function absenajax()
    {
		...
		//ambil Waktu Absen hari ini
		$hari = array("Minggu", "Senin", "Selasa", "Rabu", "Kamis", "Jumat", "Sabtu");
        $appsettings = $this->db->get_where('db_hari', ['nama_hari' => $hari[(int)date("w")]])->row();
        ...
        //     'csrfHash' => $this->security->get_csrf_hash(),
        //     'success'  => false,
        //     'msgabsen' =>  $this->appsetting['absen_mulai']
        // ];
        // echo json_encode($reponse);
        // die();
        if (strtotime($clocknow) <= strtotime($this->appsetting['absen_mulai'])) {
            $reponse = [
                'csrfName' => $this->security->get_csrf_token_name(),
                'csrfHash' => $this->security->get_csrf_hash(),
                'success'  => false,
        ...
        } elseif (strtotime($clocknow) >= strtotime($this->appsetting['absen_mulai']) && strtotime($clocknow) <= strtotime($appsettings->jam_pulang) && $this->db->get_where('db_absensi', ['tgl_absen' => $today, 'kode_pegawai' => $this->get_datasess['kode_pegawai']])->row_array()) {
            $reponse = [
                'csrfName' => $this->security->get_csrf_token_name(),
                'csrfHash' => $this->security->get_csrf_hash(),
                'success' => false,
        ...
        } elseif (strtotime($clocknow) >= strtotime($this->appsetting['absen_mulai']) && strtotime($clocknow) >= strtotime($appsettings->jam_pulang) && !empty($is_pulang['jam_pulang'])) {
            $reponse = [
                'csrfName' => $this->security->get_csrf_token_name(),
                'csrfHash' => $this->security->get_csrf_hash(),
                'success' => false,
    ...
    { //data absen by JSON object
        ...
        if ($dataabsen == 'datapgw') {
            ...
            if ($this->session->userdata('role_id') == 1) {
                ...
                foreach ($query->result() as $r) {
                    if(empty($r->status_pegawai)){
                        $txtAbsen = '<span class="badge badge-primary">Belum Absen</span>';
                    }elseif($r->status_pegawai == 1){
                        $txtAbsen = '<span class="badge badge-success">Sudah Absen</span>';
                    }elseif($r->status_pegawai == 2){
                        $txtAbsen = '<span class="badge badge-danger">Absen</span>';
                    }elseif($r->status_pegawai == 3){
                        $txtAbsen = '<span class="badge badge-warning">Absen Terlambat</span>';
                    }
                    $data[] = [
                        $no++,
                        $r->tgl_absen,
    ...
    //Fitur Ajax get data Tabel Form Izin 

    function get_datatbl2()
    { //data absen by JSON object
        $dataabsen = $this->input->get('type');
        $datapegawai = $this->get_datasess;
        $draw = intval($this->input->get("draw"));
        ...
        $no = 1;
        $check_admin = $this->db->get_where('user', ['role_id' => 1]);
        $query = $this->db->query("select * from tb_izin left join user on tb_izin.nama = user.id_pegawai");
        ...
        foreach ($query->result() as $r) {
            $data[] = [
                ...
                $r->tanggal_masuk,
                '<div class="btn-group btn-small " style="text-align: right;">
                    <button class="btn btn-danger delete-izin" title="Hapus Izin" data-izin-id="' . $r->id. '"><span class="fas fa-trash"></span></button>
                    <button class="btn btn-warning edit-izin" title="Edit izin" data-toggle="modal" data-target="#editizinmodal" data-izin-id="' . $r->id. '"><span class="fas fa-user-edit"></span></button>
                    <button class="btn btn-primary print-izin" title="Cetak Izin" data-izin-id="' . $r->id. '" data-toggle="modal" data-target="#printizinmodal"><span class="fas fa-print"></span></button>
                </div>'
            ];
        }
        $result = array(
    ...
    public function dataizin()
    {
        ...
        if ($typesend == 'addizin') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->M_Admin->crudizin($typesend);
                $reponse = [
                    'csrfName' => $this->security->get_csrf_token_name(),
                    'csrfHash' => $this->security->get_csrf_hash(),
                    'success'  => true
            ...
            if ($this->get_datasess['role_id'] != 1 || $check_admin->num_rows() < 1) {
                $reponse['message'] = 'Hanya admin yang boleh menghapus izin!';
            } else {
                $reponse = [
                    'csrfName' => $this->security->get_csrf_token_name(),
                    'csrfHash' => $this->security->get_csrf_hash(),
                    'message' => 'Anda telah menghapus izin!',
                    'success' => true
                ];
                $this->M_Admin->crudizin($typesend);
            }
        } elseif ($typesend == 'edtizin') {
            $data['dataapp'] = $this->appsetting;
            $data['dataizin'] =  $this->db->get_where('tb_izin', ['id' => $this->input->post('id_izin')])->row_array();
            $data['pegawai'] =  $this->M_Admin->dataPegawai();
            // print_r($data['dataizin']); die();
            $html = $this->load->view('layout/datapegawai/editizin', $data);
            $reponse = [
                'html' => $html,
                'csrfName' => $this->security->get_csrf_token_name(),
                'csrfHash' => $this->security->get_csrf_hash()
    ...
    public function editizinbc()
    {
        ...
        $validation = [
            ...
            [
                'field' => 'jenis_izin_edit',
                'label' => 'Jenis Izin',
                'rules' => 'required|xss_clean',
                'errors' => ['required' => 'You must provide a %s.', 'xss_clean' => 'Please check your form on %s.']
            ],
            [
                'field' => 'tanggal_izin_edit',
                'label' => 'Tanggal Izin',
                'rules' => 'required|xss_clean',
                'errors'=> ['required' => 'You must provide a %s.', 'xss_clean' => 'Please check your form on %s.']
            ],
