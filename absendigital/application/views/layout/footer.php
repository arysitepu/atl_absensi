<?php if ($this->session->userdata('logged_in') == true) : ?>
        ...
        <div class="container-fluid">
            <div class="d-flex align-items-center justify-content-between small">
                <div class="text-muted">Copyright &copy; <?= date("Y"); ?><a href="<?= base_url(); ?>" class="ml-1"><?= $appname = (empty($dataapp['nama_app_absensi'])) ? 'Absensi Online' : $dataapp['nama_app_absensi']; ?></a>
                    <div class="d-inline">By<a href="" class="ml-1 text-success">Ary Sitepu</a></div>
                </div>
    ...
    <script>
        ...
        $("#divBtnAbsen").on('click',".btnRefresh", function(e){
            loadDivAgain();
        });
        $("#divBtnAbsen").on('click',"#btn-absensi", function(e){
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success == true) {
                        ...
                        // $('#divBtnAbsen').load(location.href + " #divBtnAbsen");
                        loadDivAgain();
                    } else {
    ...
    <?php if ($this->session->userdata('role_id') == 1) : ?>
        ...
        <script>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#editpegawai').submit(function(e) {
                            e.preventDefault();
                            var form = this;
                            $("#editpegawai-btn").html("<span class='fas fa-spinner fa-pulse' aria-hidden='true' title=''></span> Menyimpan").attr("disabled", true);
                            var formdata = new FormData(form);
                            $.ajax({
                                ...
                                success: function(response) {
                                    if (response.success == true) {
                                        ...
                                        form.reset();
                                        $("#editpegawai-btn").html("<span class='fas fa-pen mr-1' aria-hidden='true' ></span>Edit").attr("disabled", false);
                                    } else {
                                        swal.close()
                                        $("#editpegawai-btn").html("<span class='fas fa-pen mr-1' aria-hidden='true' ></span>Edit").attr("disabled", false);
            ...
            //form izin
            $('#dataizinpegawai').DataTable({
                "ajax": {
                    url: "<?= base_url('ajax/get_datatbl2?type=datapgw'); ?>",
                    type: 'get',
                    async: true,
                    "processing": true,
                    "serverSide": true,
            ...
            $('#addizin').submit(function(e) {
                e.preventDefault();
                var form = this;
                $("#addizin-btn").html("<span class='fas fa-spinner fa-pulse' aria-hidden='true' title=''></span> Proses Penambahan").attr("disabled", true);
                var formdata = new FormData(form);
                $.ajax({
                    url: "<?= base_url('ajax/dataizin?type=addizin'); ?>",
                    type: 'POST',
                    data: formdata,
                    processData: false,
                    contentType: false,
                    ...
                    success: function(response) {
                        ...
                        if (response.success == true) {
                            ...
                            swal.fire({
                                icon: 'success',
                                title: 'Penambahan Izin',
                                text: 'Penambahan izin sudah berhasil !',
                                showConfirmButton: false,
                                timer: 1500
                            });
                            $('#dataizinpegawai').DataTable().ajax.reload();
                            $('#addIzinPegawai').modal('hide');
                            form.reset();
                            $("#addpgw-btn").html("<span class='fas fa-plus mr-1' aria-hidden='true' ></span>Simpan").attr("disabled", false);
                        } else {
                            swal.close()
                    ...
                    error: function() {
                        swal.fire("Penambahan Izin Gagal", "Ada Kesalahan Saat penambahan pegawai!", "error");
                        $("#addpgw-btn").html("<span class='fas fa-pen mr-1' aria-hidden='true' ></span>Edit").attr("disabled", false);
                    }
                });
            ...
            $("#dataizinpegawai").on('click', '.delete-izin', function(e) {
                e.preventDefault();
                var id_izin = $(e.currentTarget).attr('data-izin-id');
                if (id_izin === '') return;
                Swal.fire({
                    title: 'Hapus Izin Ini?',
                    text: "Anda yakin ingin menghapus Izin ini!",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    ...
                    if (result.value) {
                        $.ajax({
                            type: "POST",
                            url: '<?= base_url('ajax/dataizin?type=delizin'); ?>',
                            data: {
                                id_izin: id_izin
                            },
                            beforeSend: function() {
            ...
            $("#dataizinpegawai").on('click', '.edit-izin', function(e) {
                e.preventDefault();
                var id_izin = $(e.currentTarget).attr('data-izin-id');
                if (id_izin === '') return;
                $.ajax({
                    type: "POST",
                    url: '<?= base_url('ajax/dataizin?type=edtizin'); ?>',
                    data: {
                        id_izin: id_izin
                    },
                    beforeSend: function() {
                    ...
                    success: function(data) {
                        ...
                        $('#editdataizin').html(data);

                        $('#editizin').submit(function(e) {
                            e.preventDefault();
                            var form = this;
                            $("#editizin-btn").html("<span class='fas fa-spinner fa-pulse' aria-hidden='true' title=''></span> Menyimpan").attr("disabled", true);
                            var formdata = new FormData(form);
                            $.ajax({
                                url: "<?= base_url('ajax/editizinbc?type=edtpgwalt'); ?>",
                                type: 'POST',
                                data: formdata,
                                processData: false,
                                contentType: false,
                                ...
                                success: function(response) {
                                    if (response.success == true) {
                                        ...
                                        swal.fire({
                                            icon: 'success',
                                            title: 'Edit Izin Berhasil',
                                            text: 'Edit Izin sudah berhasil !',
                                            showConfirmButton: false,
                                            timer: 1500
                                        });
                                        $('#dataizinpegawai').DataTable().ajax.reload();
                                        $('#editizinmodal').modal('hide');
                                        form.reset();
                                        $("#editizin-btn").html("<span class='fas fa-pen mr-1' aria-hidden='true' ></span>Edit").attr("disabled", false);
                                    } else {
                                        swal.close()
                                        $("#editizin-btn").html("<span class='fas fa-pen mr-1' aria-hidden='true' ></span>Edit").attr("disabled", false);
                                        $("#info-edit").html(response.messages);
            ...
            $("#dataizinpegawai").on('click', '.print-izin', function(e) {
                e.preventDefault();
                var id_izin = $(e.currentTarget).attr('data-izin-id');
                if (id_izin === '') return;
                $('#printizinmodal').on('show.bs.modal', function(e) {
                    $(this).find('.btn-print-direct').attr('href', '<?= base_url('cetak?id_izin='); ?>' + id_izin + '');
                });
                $("#printdataizin").html('<object type="application/pdf" data="<?= base_url('cetak?id_izin='); ?>' + id_izin + '" height="850" style="width: 100%; display: block;">Your browser does not support object tag</object>');
            });
            //batas form izin
        </script>
