    ...
    <!-- <div class="mt-4 jumbotron jumbotron-fluid shadow-lg">
        <div class="container">
            <div class="text-center">
                ...
                <p class="lead">
                    <marquee width="60%" direction="left" class="ml-1 text-success font-weight-bold"><?= (empty($dataapp['jumbotron_lead_set'])) ? '[Ubah Kalimat Pada Teks Ini Disetting Aplikasi]' : $dataapp['jumbotron_lead_set']; ?></marquee>
                </p>
            </div>
        </div>
    </div> -->
            ...
            <div class="card mb-4">
                ...
                <div class="card-body text-center">
                    ...
                    <?php 
                     ...
                     if (empty($dbabsensi['keterangan_absen'])){
                            echo form_dropdown('ket_absen', ['Bekerja Di Kantor' => 'Bekerja Di Kantor'], '', ['class' => 'form-control align-content-center my-2', 'id' => 'ket_absen']); 
                        }else{
                            echo "<h4><span class='badge badge-primary'>". $dbabsensi['keterangan_absen'] ."</span></h4>";
                        }

                        // if (empty($dbabsensi['keterangan_absen'])){
                        //     echo form_dropdown('ket_absen', ['Bekerja Di Kantor' => 'Bekerja Di Kantor', 'Bekerja Di Rumah / WFH' => 'Bekerja Di Rumah / WFH', 'Sakit' => 'Sakit', 'Cuti' => 'Cuti'], '', ['class' => 'form-control align-content-center my-2', 'id' => 'ket_absen']); 
                        // }else{
                        //     echo "<h4><span class='badge badge-primary'>". $dbabsensi['keterangan_absen'] ."</span></h4>";
                        // }
                    ?>
                    <div class="mt-2">
                        <div id="func-absensi">
                            ...
                            <div id="jamabsen">
                                ...
                                <p>Waktu Pulang: <?= $jampulang = (empty($dbabsensi['jam_pulang'])) ? '00:00:00' : $dbabsensi['jam_pulang']; ?></p>
                            </div>
                        </div>
                        <div id="divBtnAbsen">
                            <?php
                                if ($datawaktu->status == 'aktif') {
                                    if (empty($dbabsensi['jam_masuk']) && strtotime(date("H:i:s")) <= strtotime($datawaktu->max_telat)) {
                                        echo "<button class='btn btn-success' id='btn-absensi'>Absen Datang</button>";
                                    }elseif(empty($dbabsensi['jam_masuk']) && empty($dbabsensi['jam_pulang']) && strtotime(date("H:i:s")) < strtotime($datawaktu->jam_pulang)){
                                        echo "<button class='btn btn-success' id='btn-absensi'>Absen Datang</button>";
                                    }elseif(!empty($dbabsensi['jam_masuk']) && empty($dbabsensi['jam_pulang']) && strtotime(date("H:i:s")) <= strtotime($datawaktu->jam_pulang)){
                                        echo "<button class='btn btn-info btnRefresh mb-2'><i class='fas fa-sync-alt'></i></button>";
                                        echo "<br /><span class='badge badge-pills badge-success'>Sudah mengisi absen masuk <br> Silahkan absen pada waktu pulang yang sudah di tentukan! </span>";
                                    }elseif(!empty($dbabsensi['jam_masuk']) && empty($dbabsensi['jam_pulang']) && strtotime(date("H:i:s")) >= strtotime($datawaktu->jam_pulang)){
                                        echo "<button class='btn btn-warning' id='btn-absensi'>Absen Pulang</button>";
                                    }elseif(!empty($dbabsensi['jam_masuk']) && !empty($dbabsensi['jam_pulang'])){
                                        echo "<span class='badge badge-pills badge-info'>Sudah mengisi absen hari ini <br> Silahkan absen pada hari berikutnya </span>";
                                    }else{
                                        echo "<span class='badge badge-pills badge-info'>Sudah melewati waktu pengisian absen hari ini <br> Silahkan absen pada hari berikutnya </span>";
                                    }
                                }else{
                                    echo "<span class='badge badge-pills badge-info'>Hari libur </span>";
                                }
                            ?>
                        </div>
                    </div>
                ...
                <div class="card-footer">
                    <div class="d-flex align-items-center justify-content-between">
                        <div class="text-muted">Absen Datang Jam: <?= $datawaktu->jam_masuk ?></div>
                        <div class="text-muted">Absen Pulang Jam: <?= $datawaktu->jam_pulang; ?></div>
                    </div>
                </div>
            </div>
        </div>
