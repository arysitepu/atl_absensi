...
class M_Admin extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT id_pegawai, nama_lengkap, kode_pegawai FROM user where is_active = 1");
        return $query->result_array();
    }

    public function listizin($table)
    {
...
//model untuk izin
public function crudizin($typesend)
    {
        if ($typesend == 'addizin') {
            $sendsave = [
                'nama' => htmlspecialchars($this->input->post('nama')),
                'divisi' => htmlspecialchars($this->input->post('divisi')),
                'jenis_izin' => htmlspecialchars($this->input->post('jenis_izin')),
                'tanggal_izin' => htmlspecialchars($this->input->post('tanggal_izin')),
                'tanggal_masuk' => htmlspecialchars($this->input->post('tanggal_masuk'))
            ];
            $this->db->insert('tb_izin', $sendsave);
        } elseif ($typesend == 'delizin') {
            $this->db->delete('tb_izin', ['id' => htmlspecialchars($this->input->post('id_izin', true))]);
        } elseif ($typesend == 'actpgw') {
            $this->db->set('is_active', 1);
            $this->db->where('id_pegawai', htmlspecialchars($this->input->post('pgw_id', true)));
            $this->db->update('user');
            ...
            $sendsave = [
                ...
                'divisi' => htmlspecialchars($this->input->post('divisi_edit')),
                'jenis_izin' => htmlspecialchars($this->input->post('jenis_izin_edit')),
                'tanggal_izin' => htmlspecialchars($this->input->post('tanggal_izin_edit')),
                'tanggal_masuk' => htmlspecialchars($this->input->post('tanggal_masuk_edit'))
            ];
            $this->db->set($sendsave);
            $this->db->where('id', htmlspecialchars($this->input->post('id_edit', true)));
            $this->db->update('tb_izin');
        }
    }

    
    public function crudizin2($typesend)
    {
        if($typesend == 'addizin'){
