...
class M_Front extends CI_Model
{
    ...
    public function do_absen()
    {
        ...
        //ambil pengaturan default setting
        $pengaturanDefault = $this->fetchsetupapp();
        
        $today = $this->get_today_date;
        $clocknow = date("H:i:s");
        if (strtotime($clocknow) >= strtotime($this->appsetting['absen_mulai']) && strtotime($clocknow) <= strtotime($appsettings->jam_masuk)) { //cek waktu, tidak telat
            if ($this->db->get_where('db_absensi', ['tgl_absen' => $today, 'kode_pegawai' => $this->get_datasess['kode_pegawai']])->row_array()) { //sudah ada data absensi maka update
                $data = [
                    'jam_masuk' => $clocknow
                ];
                $this->db->where('tgl_absen', $today)->where('kode_pegawai', $this->get_datasess['kode_pegawai']);
                $this->db->update('db_absensi', $data);
            } else { //belum ada data absensi, sehingga tambah baru data absensi
                $data = [
                    ...
                    'status_pegawai' => 1,
                    'maps_absen' => $pengaturanDefault['maps_use'] == TRUE ? htmlspecialchars($this->input->post('maps_absen', true)) : 'No Location'
                ];
                $this->db->insert('db_absensi', $data);
            }
        } elseif (strtotime($clocknow) >= strtotime($this->appsetting['absen_mulai']) && strtotime($clocknow) >= strtotime($appsettings->jam_pulang)) { // cek waktu, sudah telat
        ...
        } elseif (strtotime($clocknow) >= strtotime($this->appsetting['absen_mulai']) && strtotime($clocknow) <= strtotime($appsettings->jam_pulang)) { // kalau pulang lebih awal, maka absen
            if (strtotime($clocknow) <= strtotime($appsettings->max_telat)) {
                $data = [
                    'nama_pegawai' => $this->get_datasess['nama_lengkap'],
                    'kode_pegawai' => $this->get_datasess['kode_pegawai'],
                    'jam_masuk' => $clocknow,
